...
import artellapipe.tools.shotassembler
from artellapipe.gui import window, progressbar
from artellapipe.utils import shader, json_include, exceptions, resource
from artellapipe.tools.shotassembler.core import override
from artellapipe.tools.shotassembler.widgets import shothierarchy, shotproperties, shotassetslist, shotoverrides

logging.config.fileConfig(artellapipe.tools.shotassembler.get_logging_config(), disable_existing_loggers=False)
logger = logging.getLogger(__name__)
logger.setLevel(artellapipe.tools.shotassembler.get_logging_level())
...
class ShotFile(object):
    ...
    def build_shot(self, skip_root=False):
        ...
        try:
            ...
            logger.error('Error while loading Shot File: {} | {} | {}'.format(self._file_path, e, traceback.format_exc()))
            return dict()
        ...
        if root_shot and not skip_root and root_shot != os.path.basename(self._file_path):
            ...
            if not os.path.isfile(root_shot_file):
                logger.warning('Root Shot File does not exists: "{}"!'.format(root_shot_file))
                return dict()
            root_data = ShotFile(project=self._project, file_path=root_shot_file).load()
            if not root_data:
                logger.warning('Root Shot File "{}" has not data!'.format(root_shot_file))
                return dict()
    ...
    def save(self, asset_files, override_files):
        ...
        for asset_file in asset_files:
            ...
            if not os.path.isfile(asset_file_path):
                logger.warning('Asset File {} does not exists!'.format(asset_file_path))
                continue
        ...
        for override_file in override_files:
            ...
            if not override_file_path or not os.path.isfile(override_file_path):
                logger.warning('Override File {} does not exists!'.format(override_file_path))
                continue
            ...
            if override_name in shot_data['overrides']:
                logger.warning('Override {} is already stored. Skipping ...!'.format(override_name))
...
class ShotAssembler(window.ArtellaWindow, object):
    ...
    def ui(self):
        ...
        self._generate_btn.setIcon(resource.ResourceManager.instance().icon('magic'))
    ...
    def _update_assets(self):
        ...
        for asset in all_assets:
            ...
            if asset_name in self._assets:
                logger.warning('Asset {} is duplicated!'.format(asset_name))
                continue
    ...
    def _setup_menubar(self):
        ...
        menubar = QMenuBar()
        new_file_icon = resource.ResourceManager.instance().icon('new_file')
        load_icon = resource.ResourceManager.instance().icon('open')
        save_icon = resource.ResourceManager.instance().icon('save')
        save_override_icon = resource.ResourceManager.instance().icon('save_as')
    ...
    def add_asset(self, asset):
        ...
        if not asset_data:
            logger.warning('Asset "{}" has not valid data! Skipping ...'.format(asset))
            return
    ...
    def _on_load_shot(self):
        ...
        if not shot_data:
            logger.warning('Shot File has no data: {}!'.format(shot_file))
            return
        ...
        if not shot_files and not shot_root:
            logger.warning('Shot File has no files: {}!'.format(shot_file))
            return
        ...
        if shot_data_version != self._project.DataVersions.SHOT:
            logger.warning('Shot File {} is not compatible with current format. Please contact TD!'.format(shot_file))
            return
        if assembler_version != self.VERSION:
            logger.warning('Shot File {} was exported with an older/newer version. Please contact TD!'.format(shot_file))
            return
    ...
    def _on_save_shot(self):
        ...
        if not asset_files:
            logger.warning('No Asset Paths to export for Shot. Shot export aborted!')
            return
    ...
    def _on_save_shot_override(self):
        ...
        if not self._shot_file:
            logger.warning('No Root Shot found! Load a Shot first and try again ...')
            return
    ...
    def _on_update_hierarchy(self):
        ...
        for asset in self._shot_assets.all_assets():
            ...
            if not os.path.isfile(asset_path):
                logger.warning('Impossible to load asset asset file: {}'.format(asset_path))
                continue
