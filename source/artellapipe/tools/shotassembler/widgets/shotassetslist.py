    ...
    def load_shot_files(self, shot_files):
        ...
        for file_name, file_info in shot_files.items():
            ...
            if not os.path.isfile(file_path):
                logger.warning('Asset File "{}" does not exists in your computer!'.format(file_path))
                continue
            ...
            if not file_data:
                logger.warning('Asset File "{}" does not contains any data!'.format(file_path))
                continue
            ...
            for _, file_type in reversed(self._file_types.items()):
                if file_type and file_type.FILE_EXTENSION == file_extension:
                    ...
                    if not new_asset_file:
                        logger.warning('Impossible to generate {} asset from {}! Skipping ...'.format(file_type, file_path))
                        break
                    assets_added = True
