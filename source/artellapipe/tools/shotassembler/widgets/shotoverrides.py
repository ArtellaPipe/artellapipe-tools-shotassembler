...
class ShotOverrides(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        self._load_btn.setIcon(resource.ResourceManager.instance().icon('open'))
    ...
    def load_override_files(self, override_files):
        ...
        for file_name, file_data in override_files.items():
            ...
            if not os.path.isfile(file_path):
                logger.warning('Override File "{}" does not exists in your computer!'.format(file_path))
                continue
            if not file_data:
                logger.warning('Override File "{}" does not contains any data!'.format(file_path))
                continue
            ...
            for override_name, override in self._overrides.items():
                if override.OVERRIDE_EXTENSION == file_extension:
                    ...
                    if not new_override_file:
                        logger.warning('Impossible to generate {} override from {}! Skipping ...'.format(override_name, file_path))
                        break
